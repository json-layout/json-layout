...
describe('normalize schema fragment function', () => {
  ...
  it('should transform schema fragments with optional layout keywords in normalized layout information', () => {
    assert.deepEqual(normalize('prop', { type: 'string' }, '/prop', components).layout, defaultTextFieldComp)
    assert.deepEqual(normalize('prop', { type: 'string', layout: 'textarea' }, '/prop', components).layout, defaultTextareaComp)
  })

  it('reject an unknown component', () => {
    const normalized = normalize('prop', { type: 'string', layout: 'unknown' }, '/prop', components)
  ...
  it('should manage a layout expressed as a switch', () => {
    assert.deepEqual(
      normalize('prop', { type: 'string', layout: { switch: [{ if: 'read', comp: 'text-field' }, { if: 'write', comp: 'textarea' }] } }, '/prop', components).layout,
      {
        switch: [
  ...
  it('should calculate a label for a field', () => {
    assert.deepEqual(normalize('prop', { type: 'string' }, '/prop', components).layout, { comp: 'text-field', label: 'prop' })
    assert.deepEqual(normalize('prop', { type: 'string', title: 'Prop' }, '/prop', components).layout, { comp: 'text-field', label: 'Prop' })
    assert.deepEqual(normalize('prop', { type: 'string', title: 'Prop', layout: { label: 'Prop label' } }, '/prop', components).layout, { comp: 'text-field', label: 'Prop label' })
    assert.deepEqual(normalize('prop', { type: 'object', title: 'Prop', layout: { comp: 'select', items: [{ key: '1' }] } }, '/prop', components).layout, { comp: 'select', label: 'Prop', items: [{ key: '1', value: '1', title: '1' }] })
  })

  it('should handle number types', () => {
    assert.deepEqual(normalize('prop', { type: 'number' }, '/prop', components).layout, { comp: 'number-field', label: 'prop' })
    assert.deepEqual(normalize('prop', { type: 'number', layout: { step: 0.1 } }, '/prop', components).layout, { comp: 'number-field', label: 'prop', step: 0.1 })
    assert.deepEqual(normalize('prop', { type: 'integer' }, '/prop', components).layout, { comp: 'number-field', label: 'prop', step: 1 })
  })

  it('should accept layout as a string', () => {
    assert.deepEqual(normalize('prop', { type: 'object', layout: 'file-input' }, '/prop', components).layout, { comp: 'file-input', label: 'prop' })
    assert.deepEqual(normalize('prop', { type: 'array', layout: 'file-input' }, '/prop', components).layout, { comp: 'file-input', label: 'prop', multiple: true })
  })

  it('should handle "none" display', () => {
    assert.deepEqual(normalize('prop', { type: 'number', layout: 'none' }, '/prop', components).layout, { comp: 'none' })
  })

  it('should manage children array', () => {
    assert.deepEqual(
      normalize('prop', { type: 'object', properties: { nb1: { type: 'number' }, nb2: { type: 'number' } } }, '/prop', components).layout,
      { comp: 'section', title: null, children: [{ key: 'nb1' }, { key: 'nb2' }] }
    )
    assert.deepEqual(
      normalize('prop', { layout: ['nb1'], type: 'object', properties: { nb1: { type: 'number' }, nb2: { type: 'number' } } }, '/prop', components).layout,
  ...
  it('should manage select on enums and oneOf', () => {
    assert.deepEqual(
      normalize('prop', { type: 'string', enum: ['val1', 'val2'] }, '/prop', components).layout,
      {
        comp: 'select',
        label: 'prop',
    ...
    assert.deepEqual(
      normalize('prop', { type: 'string', oneOf: [{ const: 'val1', title: 'Val 1' }, { const: 'val2' }] }, '/prop', components).layout,
      {
        comp: 'select',
        label: 'prop',
    ...
    assert.deepEqual(
      normalize('prop', { type: 'string', layout: { items: [{ value: 'val1', title: 'Val 1' }, 'val2'] } }, '/prop', components).layout,
  ...
  it('should manage select with getItems', () => {
    assert.deepEqual(
      normalize('prop', { type: 'string', layout: { getItems: 'context.items' } }, '/prop', components).layout,
  ...
  it('should manage select with getItems from URL', () => {
    assert.deepEqual(
      normalize('prop', { type: 'string', layout: { getItems: { url: 'http://test.com/${parent.data.path}' } } }, '/prop', components).layout,
  ...
  it('should manage select on arrays with enums and oneOf', () => {
    assert.deepEqual(
      normalize('prop', { type: 'array', items: { type: 'string', enum: ['val1', 'val2'] } }, '/prop', components).layout,
      {
        comp: 'select',
        label: 'prop',
        multiple: true,
  ...
  it('should manage a select of subtypes based on a oneOf', () => {
    ...
    assert.deepEqual(
      normalize('prop', schema, '/prop', components).layout,
      {
        comp: 'section',
        title: 'Subtypes section',
    ...
    assert.deepEqual(
      normalize('prop', schema, '/prop', components, undefined, undefined, 'oneOf').layout,
      {
        comp: 'one-of-select'
      }
    )
  ...
  it('should manage combobox with examples on simple types', () => {
    assert.deepEqual(
      normalize('prop', { type: 'string', examples: ['val1', 'val2'] }, '/prop', components).layout,
  ...
  it('should accept lib specific props, slots and options', () => {
    assert.deepEqual(
      normalize('prop', { type: 'string', layout: { props: { prop1: 'Prop 1' }, slots: { before: 'Before **slot**', component: 'slot-comp' }, options: { opt1: 'Opt 1' }, opt2: 'Opt 2' } }, '/prop', components, (str) => `markdown: ${str}`, ['opt1', 'opt2']).layout,
      {
        comp: 'text-field',
        label: 'prop',
        props: { prop1: 'Prop 1' },
  ...
  it('should manage tabs layout', () => {
    assert.deepEqual(
      normalize('prop', { type: 'object', layout: { comp: 'tabs', children: [{ title: 'Tab 1', children: ['str1'] }, { title: 'Tab 2', children: ['str2'] }] }, properties: { str1: { type: 'string' }, str2: { type: 'string' } } }, '/prop', components).layout,
  ...
  it('should accept titles and subtitles for sections', () => {
    assert.deepEqual(
      normalize('prop', { type: 'object', title: 'Title', layout: { subtitle: 'A subtitle' }, properties: {} }, '/prop', components).layout,
      { comp: 'section', title: 'Title', subtitle: 'A subtitle', children: [] }
    )
    assert.deepEqual(
      normalize('prop', { type: 'object', layout: { title: 'Title' }, properties: {} }, '/prop', components).layout,
      { comp: 'section', title: 'Title', children: [] }
    )
    assert.deepEqual(
      normalize('prop', { type: 'object', title: 'Title', layout: { title: null }, properties: {} }, '/prop', components).layout,
  ...
  it('should manage a list of pattern properties', () => {
    ...
    assert.deepEqual(
      normalize('prop', schema, '/prop', components, undefined, undefined, 'patternProperties').layout,
      {
        comp: 'list',
        indexed: ['.*'],
        title: 'Pattern properties section',
