...
- `CompiledLayout.schema` — the core-cloned, core-decorated schema object. May be walked by JSON pointer to recover raw user-authored fields (`title`, `description`, etc.) that normalization rewrites or drops (e.g. `title` → `label` with a key fallback, `description` → HTML-wrapped `help`). Must NOT be walked for anything derivable from the other artifacts above — use them first. This carve-out exists because v1's completion/help surface needs the pre-normalization strings, and core does not yet expose them by another channel.
